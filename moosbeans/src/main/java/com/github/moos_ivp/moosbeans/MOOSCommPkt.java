...
/**
 * Currently no Zip compression is supported. It will throw an exception if a
 * compressed packet is tried to decode. This is not a like for like C++ port.
 * This uses java naming conventions. However, the methods for decoding are
 * still serialize() in lower case.
 *
 * packet layout int [pktLenghtInt][pktLenghtInt][pktLenghtInt][pktLenghtInt]
 * int [numberMessages][numberMessages][numberMessages][numberMessages] byte
 * [compressed?] //next packetLengthInBytes worth of packet payload: //Message 0
 * from byte 0+4 onwards. int
 * [msgLengthInt][msgLengthInt][msgLengthInt][msgLengthInt] int
 * [msgIDInt][msgIDInt][msgIDInt][msgIDInt] byte [msgType] byte [dataType] int
 * [srcLengthInt][srcLengthInt][srcLengthInt][srcLengthInt] string [String * n
 * Bytes] ? optional int [SrcAuxInt][SrcAuxInt][SrcAuxInt][SrcAuxInt] ? optional
 * [String * n Bytes] int
 * [originCommunityLengthInt][originCommunityLengthInt][originCommunityLengthInt][originCommunityLengthInt]
 * string [String * n Bytes] int [m_sKeyInt][m_sKeyInt][m_sKeyInt][m_sKeyInt]
 * string [String * n Bytes] double time
 * [time][time][time][time][time][time][time][time] double m_dfVal
 * [m_dfVal][m_dfVal][m_dfVal][m_dfVal][m_dfVal][m_dfVal][m_dfVal][m_dfVal]
 * double m_dfVal
 * [m_dfVal2][m_dfVal2][m_dfVal2][m_dfVal2][m_dfVal2][m_dfVal2][m_dfVal2][m_dfVal2]
 * int [m_sValInt][m_sValInt][m_sValInt][m_sValInt] string [String * n Bytes]
 * ... next message
 *
...
public class MOOSCommPkt {
    ...
    public final void resetFill() {
        ...
        packetData = Utils.allocate(bytesRequired); // ready for first data
    ...
    /**
     * Once this method has been called, call serialise(list, false) or
     * deSerialize() to use internal message list.
     *
            ...
            if (packetData.limit() > 3) {
                ...
                if (length > 0) {
                    ...
                    if (packetData.limit() == this.packetLengthInBytes) {
                        ...
                        this.packetData = Utils.allocate(this.packetLengthInBytes).put(packetData); // create new buffer of correct length for full packet and add already collected data
    ...
    /**
     * Convenience method to serialize from the list of messages held by this
     * packet, or use deSerialize() to deserialise to the list of messages held
     * by this packet.
     *
    ...
    /**
     * This method will either serialize the data in the messages list ready for
     * sending, or it will decode the byte[] which backs this buffer, supplied
     * by the fill() method, and add all the messages contained in this packet
     * to he supplied message list.
     *
     * @param messages the list of messages to be added to or to be serialized
     * into this packets backing byte[]. This list should be non-null and should
     * be empty if this method is to add messages from this packet to the list.
     * @param toStream whether this should serialize the supplied messages to
     * the backing byte[] or whether it should read data and populate the
     * message list with the new messages.
    ...
    public boolean serialize(ArrayList<MOOSMsg> messages, boolean toStream) {
        ...
        if (toStream) {
            ...
            packetData = Utils.allocate(this.packetLengthInBytes);
            ...
            if (packetData.position() != MOOSCommPkt.PACKET_HEADER_SIZE) {
                packetData.position(MOOSCommPkt.PACKET_HEADER_SIZE); // if for some reason not in correct place to start reading messages from
            }
            for (int i = 0; i < msgCount; i++) {
                MOOSMsg msg = new MOOSMsg();
                if (msg.serialize(packetData, toStream) > 0
                        && !msg.isType(MessageType.Null)) {
                    /// @todo There is probablya better way to check if a
                    /// message is valid before adding it to the messages.
                    messages.add(msg);
                }
            }
    ...
    /**
     *
     * @return returns the ByteBuffer created from the messages. Returns null is
     * serialize(..) hasn't been called, and a such the packet is empty.
     */
    ...
    /**
     * Replace the backing ByteBuffer with the selected one. Fill() should be
     * called followed by deSerialize or serialize(messages, false)
     *
    ...
    /**
     * Serialize() should be called after this, serialize(List of {@link MOOSMsg} list,
     * true) instead.
     *
    ...
    /**
     *
     * @return length of packet in bytes. Will only return header length if
     * serialize(..) hasn't been called yet.
     */
